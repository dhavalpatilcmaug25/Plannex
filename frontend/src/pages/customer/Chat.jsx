...
const Chat = () => {
  ...
  // State from navigation (if any) - this sets the initial active chat
  ...
  const [selectedChat, setSelectedChat] = useState(
    initialRecipientId
      ? { id: initialRecipientId, name: initialRecipientName }
      : null,
  );
  ...
  const loadConversations = async () => {
    try {
      ...
      // If we have an initial recipient but they are not in the list, add them temporarily to UI or rely on them appearing after first message
      if (
        initialRecipientId &&
        !data.find((c) => c.id === initialRecipientId)
      ) {
        // Determine logic: Maybe we just rely on selectedChat state
      }
    } catch (error) {
  ...
  const loadMessages = async (otherUserId) => {
    try {
      ...
      const myId = user?.id; // Assuming user object has ID.
        ...
        <Col md={4}>
          <Card className="card-modern h-100">
            ...
            <div className="list-group list-group-flush">
              ...
              {selectedChat &&
                !conversations.find((c) => c.id === selectedChat.id) && (
              ...
              {conversations.map((conv) => (
                <button
                  ...
                  onClick={() =>
                    setSelectedChat({ id: conv.id, name: conv.name })
                  }
                >
                  <div className="d-flex w-100 justify-content-between">
                    <h6 className="mb-1">
              ...
              {conversations.length === 0 && !selectedChat && (
                <p className="p-3 text-muted text-center">No active chats.</p>
              )}
            </div>
          </Card>
        </Col>
            ...
            <Card.Body className="d-flex flex-column">
              {selectedChat ? (
                <>
                  ...
                  >
                    {messages.length === 0 ? (
                      <p className="text-center text-muted mt-5">Say hello!</p>
                    ) : (
                      messages.map((msg) => (
                        <div
                          key={msg.id}
                          className={`d-flex mb-3 ${msg.isMe ? "justify-content-end" : ""}`}
                        >
                          <div
                            className={`p-3 rounded-3 shadow-sm ${msg.isMe ? "bg-primary text-white" : "bg-light"}`}
                            style={{ maxWidth: "70%" }}
                          >
